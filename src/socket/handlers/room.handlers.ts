...
const roomHandlers = (io: Server, socket: Socket) => {
    ...
    socket.on("GET_AVAILABLE_ROOMS", async ({ gameId, totalSlots, duration, bet }) => {
        console.log(6)
        if(!gameId || !totalSlots || !duration || bet === undefined || bet === null){
            socket.emit("FIELDS_ERROR", { statusCode: 200, message: 'Please send gameId, totalSlots, duration and bet value', success: false });
            return;
        }
